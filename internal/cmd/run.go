...
func Run(ctx context.Context, config ServerConfig) error {
	...
	err = loader.Register(&kongConfigWS.KongKeySetLoader{Client: grpcClients.KeySet})
	if err != nil {
		return fmt.Errorf("failed to register key-sets configuration loader: %w", err)
	}
	...
	if err := vc.AddConfigTableUpdates(kongConfigWS.ChangeRegistry.GetUpdates()); err != nil {
...
func setupGRPCClients(cc *grpc.ClientConn) grpcClients {
	return grpcClients{
		...
		KeySet:        v1.NewKeySetServiceClient(cc),
