...
func TestReporterOnce(t *testing.T) {
	...
	want := map[string]string{
		"v":   "koko.version",
		"id":  "6acb7447-eedf-4815-a193-d714c5108f7b",
		"hn":  hostInfo.Hostname,
		"osv": hostInfo.Platform + " " + hostInfo.PlatformVersion,
	}
...
func TestReporterSendStart(t *testing.T) {
	...
	hostInfo, err := host.Info()
	assert.NoError(err)
	serialized := fmt.Sprintf("<14>signal=koko-start;uptime=0;v=koko.version;"+
		"id=6acb7447-eedf-4815-a193-d714c5108f7b;hn=%s;osv=%s %s;",
...
func TestReporterSendPing(t *testing.T) {
	...
	hostInfo, err := host.Info()
	assert.NoError(err)
	serialized := fmt.Sprintf("<14>signal=koko-ping;uptime=42;v=koko.version;"+
		"id=6acb7447-eedf-4815-a193-d714c5108f7b;hn=%s;osv=%s %s;",
		hostInfo.Hostname, hostInfo.Platform, hostInfo.PlatformVersion)
	received, ok := <-reqs
	assert.True(ok)
...
func TestReporterRun(t *testing.T) {
	...
	hostInfo, err := host.Info()
	assert.NoError(err)
	staticInfo := fmt.Sprintf("v=koko."+
		"version;id=6acb7447-eedf-4815-a193-d714c5108f7b"+
		";hn=%s;osv=%s %s;",
		hostInfo.Hostname, hostInfo.Platform, hostInfo.PlatformVersion)
	...
	go func() {
		...
		serializedContent := []string{
			"<14>signal=koko-start;uptime=0;" + staticInfo,
			"<14>signal=koko-ping;uptime=1;" + staticInfo,
			"<14>signal=koko-ping;uptime=2;" + staticInfo,
		}
